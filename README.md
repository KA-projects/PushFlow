...
## Насколько это надёжный и нагруженный сервис

Проверено реальной нагрузкой (k6, 50 одновременных пользователей, 10 000+ запросов):

- **0% ошибок** — каждый запрос принят и поставлен в очередь (`100%` успешных ответов `200`).
- **~84 RPS** — стабильная пропускная способность в течение всего теста.
- **~450 мс** — среднее время ответа, **0 потерянных запросов** при пиковой нагрузке.
- Отправка асинхронная: приём запроса и доставка уведомлений разделены и не мешают друг другу.

Подробности и методика — в разделе [Результаты нагрузочного теста](#результаты-нагрузочного-теста).
...
Что проверяли: запустили `EXPECTED=100 PROVIDER=webpush bash stress/run.sh` — это 50
одновременных виртуальных пользователей (VU), которые 2 минуты слали запросы
на `/api/push/send` на живом Docker-стеке. Всего пришло 10 074 запроса.

Как сервис справился:

- **Ошибок нет.** Все 10 074 запроса обработаны успешно: каждый получил ответ `200`
  и поставил уведомление в очередь (`queued > 0`).
- **Пропускная способность:** ~84 RPS (запросов в секунду) на протяжении всего теста.
- **Скорость ответа:** в среднем ~450 мс, но чем дальше «хвост» — тем медленнее:
  95% запросов уложились в ~740 мс (цель была 500 мс, поэтому этот порог не пройден),
  самый медленный запрос занял ~915 мс.

### Что это значит

- Сервис **надёжно принимает нагрузку** — при 50 одновременных пользователях не теряет
  ни одного запроса. Для «записал в очередь и вернул 200» это хороший результат.
- **Быстрее отвечать ему мешает запись в базу, а не отправка уведомлений.** Каждый
  запрос прямо при приёме создаёт в БД записи уведомлений и задачи для воркеров, а это
  самый медленный этап. Сама отправка происходит потом, асинхронно в фоне, и на скорость
  ответа не влияет.
- **Как ускорить:** если понадобится отдавать ответ быстрее, стоит записывать всё в БД
  пакетами (массовый INSERT), а не построчно. Либо переложить и эту работу в очередь,
  оставив на приёме минимум.
